...
echo "=== GitHub SSH setup for MyPortfolio ==="
echo ""
...
if [ ! -f "$PUB_FILE" ]; then
  ...
  mkdir -p "$HOME/.ssh"
  chmod 700 "$HOME/.ssh"
  ssh-keygen -t ed25519 -C "github-elranlevy-myportfolio" -f "$KEY_FILE" -N ""
  echo "Key created."
else
...
echo "2. Title: e.g. MacBook MyPortfolio"
...
cd "$REPO_DIR"
git remote set-url origin git@github.com:elranlevy/MyPortfolio.git
echo ""
